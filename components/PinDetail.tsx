    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-4 mb-4">
          {pin.image_url ? (
            <img src={pin.image_url} alt={pin.name} className="w-16 h-16 rounded-full object-cover border-2 border-white/20" />
          ) : (
        ...
        <div className="space-y-2 text-sm">
          {pin.telegram && (
            <div className="flex items-center gap-2">
              ...
              <a href={`https://t.me/${pin.telegram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">{pin.telegram}</a>
            </div>
          )}
          {pin.ens && (
            <div className="flex items-center gap-2">
          ...
          {pin.email && (
            <div className="flex items-center gap-2">
              ...
              <a href={`mailto:${pin.email}`} className="text-blue-400 hover:underline">{pin.email}</a>
            </div>
          )}
        </div>
      </div>
